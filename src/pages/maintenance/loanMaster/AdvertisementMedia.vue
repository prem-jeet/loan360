...
const advertisementMediaTemp = ref<AdvertisementMedia[]>([]);
...
const newSouce = reactive<AdvertisementMedia>({
  name: '',
  id: null,
  createdOn: '',
  inactive: false,
...
const filteredData = computed(() =>
  advertisementMedia.value.filter((item) => item.inactive === checkBox.value)
);

const setFormData = () => {
  const index = advertisementMedia.value.findIndex(
    (obj) => obj.id === editingRowId.value
  );
  newSouce.name = index >= 0 ? advertisementMedia.value[index].name : '';
};
...
const saveEdited = async () => {
  const temp = advertisementMediaTemp.value.filter(
...
const loadSource = async () => {
  ...
  if (rsp.data) {
    ...
    advertisementMediaTemp.value = advertisementMedia.value;
  }
...
watch(name, () => {
  ...
  const temp = advertisementMediaTemp.value.find(
...
watch(nameSearchQuery, () => {
  advertisementMedia.value = advertisementMediaTemp.value.filter((item) => {
    return item.name
      .toLowerCase()
