...
const loanGroupTemp = ref<LoanGroup[]>([]);
...
const newSouce = reactive<LoanGroup>({
  name: '',
  id: null,
  createdOn: '',
  inactive: false,
...
const filteredData = computed(() =>
  loanGroup.value.filter((item) => item.inactive === checkBox.value)
);

const setFormData = () => {
  const index = loanGroup.value.findIndex(
    (obj) => obj.id === editingRowId.value
  );
  newSouce.name = index >= 0 ? loanGroup.value[index].name : '';
};
...
const saveEdited = async () => {
  const temp = loanGroupTemp.value.filter(
...
const changeActiveConfirm = async (id: number, state: boolean) => {
  ...
  const rsp = await api.put('/loanGroup/' + str, payLoad);
  if (rsp.data) {
...
const loadSource = async () => {
  ...
  if (rsp.data) {
    ...
    loanGroupTemp.value = loanGroup.value;
  }
...
watch(name, () => {
  ...
  const temp = loanGroupTemp.value.find(
...
watch(nameSearchQuery, () => {
  loanGroup.value = loanGroupTemp.value.filter((item) => {
    return item.name
      .toLowerCase()
