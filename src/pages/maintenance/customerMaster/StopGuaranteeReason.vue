...
const stopGuaranteeReasonTemp = ref<StopGuaranteeReason[]>([]);
...
const newSouce = reactive<StopGuaranteeReason>({
  name: '',
  id: null,
  createdOn: '',
  inactive: false,
...
const changeActiveConfirm = async (id: number, state: boolean) => {
  ...
  const rsp = await api.put('/stopGuaranteeReason/' + str, payLoad);
  if (rsp.data) {
